"""
BD-SHS Attribution Analysis - Step 2
======================================
Subword-level attribution via Integrated Gradients (captum) and SHAP.
Runs on fine-tuned BanglaBERT / XLM-R checkpoints from bdshs_pipeline.py.

Run:  python bdshs_attribution.py
Outputs: ./outputs/attribution/
...
LOG_FILE = OUT_DIR / "attribution_log.txt"
...
random.seed(SEED)
# Use the full cleaned test split for both attribution methods.
...
# --- Fix 2: compute [UNK] tokenization statistics per model/tokenizer
def compute_unk_rate(tokenizer, texts, max_length=128):
    unk_id = getattr(tokenizer, 'unk_token_id', None)
    if unk_id is None:
        return {"unk_token_rate": 0.0, "pct_examples_with_unk": 0.0, "total_unk_tokens": 0, "total_tokens": 0}
    total_tokens = 0
    unk_tokens = 0
    examples_with_unk = 0
    for text in texts:
        ...
        if n_unk > 0:
            examples_with_unk += 1
    return {
        "unk_token_rate": (unk_tokens / total_tokens) if total_tokens else 0.0,
        "pct_examples_with_unk": (100.0 * examples_with_unk / len(texts)) if texts else 0.0,
        "total_unk_tokens": unk_tokens,
        "total_tokens": total_tokens,
    }

# Try to compute UNK stats for configured models (if tokenizers exist). Save a small report.
unk_report = {}
for mn, mp in MODEL_CONFIGS.items():
    try:
        if not mp.exists():
            print(f"[SKIP] tokenizer not found for {mn}: {mp}")
            continue
        tok = AutoTokenizer.from_pretrained(str(mp))
        stats = compute_unk_rate(tok, test_df[TEXT_COL].tolist())
        unk_report[mn] = stats
        print(f"{mn} UNK stats: {stats}")
    except Exception as e:
        print(f"[WARN] computing UNK stats for {mn}: {e}")

if unk_report:
    with open(OUT_DIR / 'unk_rate_report.json', 'w', encoding='utf-8') as uf:
        json.dump(unk_report, uf, ensure_ascii=False, indent=2)
    print(f"UNK report -> {OUT_DIR / 'unk_rate_report.json'}")
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
print("\n" + "=" * 60)
print("SECTION 6 - ATTRIBUTION REPORT")
print("=" * 60)

rp = OUT_DIR / "attribution_report.md"
with open(rp, "w", encoding="utf-8") as f:
    f.write("# BD-SHS Attribution Analysis Report\n\n")
    ...
    f.write("## Output Files (in outputs/attribution/)\n\n")
    ...
    f.write("- attribution_log.txt — full log\n")
