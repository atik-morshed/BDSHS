...
Hierarchical aggregation can select more actual words than the nominal budget. For example, selecting a merged phrase "word1 word2" counts as 1 unit but uses 2 words. This creates an unfair comparison with flat strategies that select individual words.
...
Hierarchical aggregation achieves significantly higher actual word coverage than flat strategies at the same nominal budget:

- **10% budget**: Hierarchical (0.3134) vs Flat (0.1692) = **85% higher coverage**
- **20% budget**: Hierarchical (0.3334) vs Flat (0.2132) = **56% higher coverage**
- **30% budget**: Hierarchical (0.3699) vs Flat (0.2790) = **33% higher coverage**

This is expected because hierarchical units can contain multiple words (e.g., "খুব খারাপ" as a merged unit).
...
The coverage analysis confirms that hierarchical aggregation achieves higher actual word coverage than flat strategies. A full equal-coverage evaluation would provide the fairest comparison but requires significant computation time.

**Phase 9 Status**: ⚠ PARTIAL COMPLETE (Analysis documented, full re-evaluation pending)
